  ...
  const handleNext = () => {
    if (
      ...
      Toast.show({
        ...
        text2: 'Please fill all fields and upload at least one photo.',
      });
      return;
    }
    ...
    Toast.show({
      ...
      text2: 'Your info has been sent to admin. Kindly wait for verification.',
    });
  };

  const formatVehicleNumber = (text: string) => {
    let raw = text.replace(/[^a-zA-Z0-9]/g, "");
    if (raw.length <= 3) return raw;
    if (raw.length <= 7) return `${raw.slice(0, 3)}-${raw.slice(3)}`;
    return `${raw.slice(0, 3)}-${raw.slice(3, 7)}`; // Max 8 characters with hyphen
  };
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.photoRow}>
          {vehiclePhotos.map((uri, index) => (
            <TouchableOpacity key={index} onPress={() => handleRetakeOrDelete(index)}>
...
const styles = StyleSheet.create({
  container: {
    ...
    backgroundColor: "#fff",
    flexGrow: 1,
    paddingTop: 10, // FIXED: Reduced space above
  },
  label: {
    fontSize: 16,
    marginTop: 16,
